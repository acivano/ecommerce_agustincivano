...
const ItemCart = ({item}) => {
    ...
    function modificarEstadoCarrito(count){
        ...
        <div key={item.id} className="d-flex flex-row justify-content-between align-items-center border-bottom">
            ...
            <div className="d-flex flex-column columnaNombreCarrito w-50">
                ...
                <div className="d-flex flex-row justify-content-between align-items-end">
                    <h4 className=" text-start mt-1 w-75">{`${item.cantidad} u. x ${redondeo(item.precioventa)} =  `}<span className='h2'>{redondeo(item.precioventa * item.cantidad)}</span></h4>   
                </div>
                <div className="form-floating mb-3 w-50 align-self-center d-flex flex-row justify-content-between">
                    <ItemCount producto={item} agregarProdCarrito={modificarEstadoCarrito} initial={item.cantidad} origin={origin}/>
